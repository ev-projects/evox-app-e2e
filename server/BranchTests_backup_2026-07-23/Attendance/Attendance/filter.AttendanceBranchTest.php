...
class AttendanceFilterBranchTest extends TestCase
{
    ...
    {
        return [
            $this->bindMock(AttendanceRepositoryInterface::class),
            $this->bindMock(AttendanceGeoGate::class),
        ];
    }
    ...
    {
        if (!$this->geo) $this->markTestSkipped('no utc_timelog geo row');
    ...
    {
        if (!$this->department) $this->markTestSkipped('no EVOX_DEPARTMENT row');
        [$repo, $gate] = $this->mockDeps();
