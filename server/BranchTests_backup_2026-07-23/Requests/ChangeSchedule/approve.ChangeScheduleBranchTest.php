...
class ChangeScheduleApproveBranchTest extends TestCase
{
    ...
    public function decline__approve__previously_approved__ok_200()
    {
        ...
        if (!$cs) $this->markTestSkipped('no approved ChangeSchedule fixture');
    ...
    {
        $cs = ChangeSchedule::whereHas('user')->whereHas('schedule')->where('status', '!=', 'approved')->first();
        if (!$cs) $this->markTestSkipped('no non-approved ChangeSchedule fixture');
    ...
    {
        $this->bindMock(ChangeScheduleRepositoryInterface::class); // decline() never reached
