...
class AlterLogApproveBranchTest extends TestCase
{
    ...
    public function approve__approve__success__ok_200()
    {
        ...
        $dtr->shouldReceive('apply_alter_log_to_dtr')->once()->andReturnNull(); // mocked: no real DTR write

        $res = $this->actingAs($this->user)
                    ->putJson("/api/request/alter_log/approve/{$al->id}", $this->validPayload());
    ...
    {
        $repo = $this->bindMock(AlterLogRepositoryInterface::class);
        $repo->shouldReceive('approve')->once()->andThrow(new Exception('boom')); // request_validity=false first (old date)
        $this->bindMock(DtrRepositoryInterface::class);

        $res = $this->actingAs($this->user)
                    ->putJson('/api/request/alter_log/approve/1', $this->validPayload());
