...
class CodeOfConductLoadBranchTest extends TestCase
{
    ...
    public function index__load__no_eva_registration__ok_200_empty()
    {
        ...
        if (!$user) {
            $this->markTestSkipped('no active user without an eva_registration row in test DB');
        }
    ...
    public function index__load__has_eva_registration__ok_200()
    {
        ...
        if (!$user) {
            $this->markTestSkipped('no user with a non-deleted eva_registration row in test DB');
        }
