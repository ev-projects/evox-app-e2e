...
class OpsScheduleController extends Controller
{
    public function getList($dept_id = null)
    {
        ...
        foreach ($ops_sched_list as $key => $value) {
            ...
            if ($item['type'] == 'Form') {
                ...
                $item['start_end_time'] = $item['start_time'] . ' - ' . $item['end_time'];
            } else {
                $item['scope'] = [];
                $item['path'] = env('ASSET_URL') . Storage::url($item['path']);
            }
    ...
    public function get()
    {
        ...
        foreach ($ops_depts as $key => $value) {
            // check first if there is an image uploaded
            $image_per_dept = OpsSchedule::where('department_id', $value['id'])->where('type', 'image')->first();

            if ($image_per_dept) {
                $ops_scheds[$key]['department'] = $value['name'];
                $ops_scheds[$key]['description'] = $value['description'];
                $ops_scheds[$key]['type'] = 'image';
                $ops_scheds[$key]['image'] = env('ASSET_URL') . Storage::url($image_per_dept->path);
            } else {
                $list_per_dept = OpsSchedule::where('department_id', $value['id'])->get();
                if (count($list_per_dept)) {
                    $ops_scheds[$key]['department'] = $value['name'];
                    $ops_scheds[$key]['description'] = $value['description'];
                    $ops_scheds[$key]['type'] = 'form';
    ...
    public function store(Request $request)
    {
        DB::beginTransaction();
        try {
            // check if uploaded schedule is image or form
            if ($request->type === 'image') {
                ...
                // check if there is an existing schedule image for a specific department
                $check_sched = OpsSchedule::where('department_id', $request->department)->where('type', 'image')->first();
                if (!$check_sched) {
                    $new_ops_sched = OpsSchedule::create($data);

                    // upload of image in storage dir and update the record with the path of the image
                    $imageName = time() . '.' . $request->image->extension();
                    $path = 'public/opsschedules/' . $new_ops_sched->id . '/';
                    $request->image->storeAs($path, $imageName);
                    $new_ops_sched->update([
                        'path' => $path . $imageName,
                    ]);
                } else {
                    // upload of image in storage dir and update the record with the path of the image
    ...
    public function update(Request $request, $ops_sched_id = null)
    {
        ...
        try {
            ...
            if ($request->type === 'image') {
                // upload of image in storage dir and update the record with the path of the image
                $imageName = time() . '.' . $request->image->extension();
                $path = 'public/opsschedules/' . $ops_sched_id . '/';
                $request->image->storeAs($path, $imageName);
                $updated_ops_sched = OpsSchedule::where('id', $ops_sched_id)->update([
                    'path' => $path . $imageName,
                ]);
                DB::commit();
            } elseif ($request->type === 'form') {
