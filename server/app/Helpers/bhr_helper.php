...
if (! function_exists('bhr_api_call_bulgaria')) {  
    /**
     * This function conducts a cURL on Bamboo HR with specific API End Point.
     *
     * @param  string method
     ...
     * @return array;
     */
    function bhr_api_call_bulgaria($method = 'GET', $api_endpoint,  $data = array(), $send_as_json = false) {
        try{
            $result = null;
            ...
            # Create the Response variable that handles the Curl Request
            $response = Curl::to( env('BHR_API_LINK_BULGARIA') . $api_endpoint )
                            ->withHeader('Accept: application/json')
                            ->withTimeout(300)
                            ->withConnectTimeout(300)
                            ->returnResponseObject();
