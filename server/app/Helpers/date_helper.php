...
if (! function_exists('timestamp_to_datetime')) {   
    ...
    function timestamp_to_datetime( $timestamp, $use_OWNER = false, $owner = null ) 
    {
        ...
        if(Auth::user() && Auth::user()->country_timezone_to_offset() != null){
                ...
                if ($owner) {
                    $target_date_offset = string_offset_to_seconds($owner->country_zone()->time_difference);
                }
                // if ($owner && ($owner->country_timezone_name() == Auth::user()->country_timezone_name())) {
...
if (! function_exists('timestamp_to_time')) {   
     ...
     * @return datetime
     */
    function timestamp_to_time( $timestamp, $use_OWNER = false, $owner = null )
    {
        try {
