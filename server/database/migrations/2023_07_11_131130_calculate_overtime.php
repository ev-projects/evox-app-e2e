...
class CalculateOvertime extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        //
        DB::unprepared("
        DROP TRIGGER IF EXISTS overtimes_without_schedule;
        CREATE TRIGGER overtimes_without_schedule AFTER UPDATE ON dtr_collective_punch FOR EACH ROW BEGIN
...
DECLARE v_timeid bigint;
 ...
 DECLARE ot_status varchar(20);
 DECLARE ot_type varchar(20);
 DECLARE o_amount bigint ;
 DECLARE v_log_out_type varchar(20);
 DECLARE UIV_Date Date;
 DECLARE UIV_User_Id INT(10);
 
 SET UIV_Date = NEW.date;
 SET UIV_User_Id = NEW.user_id;
 ...
 SET v_log_out_type = (Select log_out_type from dtr_collective_punch_history where id=NEW.dtr_collective_punch_history_id);
  
 IF(v_log_out_type = 'Log_out')THEN
 ...
 SET ot_status = (select status from overtimes where user_id=NEW.user_id and date=NEW.date);
  select type,amount into ot_type,o_amount from overtimes where user_id=NEW.user_id and date=NEW.date;
 -- select amount into o_amount from overtimes where  user_id=UIV_User_Id and date=UIV_Date;
...
IF(ot_status='declined' and Not exists (Select 1 From overtimes where status='approved' and date=New.date and user_id=new.user_id and amount > 0 ))
THEN
...
END IF;
 ...
 IF(ot_status='approved') THEN

#If the Overtime Type is a Pre-Overtime.
   IF(ot_type = 'pre_overtime')THEN
    ...
    SET overtime_start_datetime = start_time - o_amount;
...
ELSEIF(ot_type = 'post_overtime')THEN

     SET start_time = v_timeout;
     SET overtime_start_datetime = start_time;
     SET overtime_end_datetime   = start_time + 3600;
