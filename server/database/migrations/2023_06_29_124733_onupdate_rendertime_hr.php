...
class OnupdateRendertimeHr extends Migration
{
    ...
    public function up()
    {
        //
        DB::unprepared("
        DROP TRIGGER IF EXISTS dtr_collective_punch;
...
SET rendered_hrs = NEW.render_hours / 3600;
SET rendered_hrs_overlapped = NEW.render_hours_overlapp / 3600;
...
IF(dtr_type = 'reg')THEN 
UPDATE drt_summary_report SET drt_summary_report.reg_rendered_hours = drt_summary_report.reg_rendered_hours + rendered_hrs,
drt_summary_report.reg_rendered_hours_overlapp =drt_summary_report.reg_rendered_hours_overlapp + rendered_hrs_overlapped,
drt_summary_report.updated_at=now() 
WHERE drt_summary_report.user_id=NEW.user_id AND drt_summary_report.login_date=NEW.date;
ELSEIF(dtr_type = 'rd')THEN 
UPDATE drt_summary_report SET drt_summary_report.rd_rendered_hours = drt_summary_report.rd_rendered_hours + rendered_hrs,
drt_summary_report.rd_rendered_hours_overlapp = drt_summary_report.rd_rendered_hours_overlapp + rendered_hrs_overlapped,
drt_summary_report.updated_at=now() 
WHERE drt_summary_report.user_id=NEW.user_id AND drt_summary_report.login_date=NEW.date;
ELSEIF(dtr_type = 'sh')THEN 
...
UPDATE drt_summary_report SET drt_summary_report.sh_rendered_hours = drt_summary_report.sh_rendered_hours + rendered_hrs,
drt_summary_report.sh_rendered_hours_overlapp = drt_summary_report.sh_rendered_hours_overlapp + rendered_hrs_overlapped,
drt_summary_report.updated_at=now() 
WHERE drt_summary_report.user_id=NEW.user_id AND drt_summary_report.login_date=NEW.date;
...
END IF; 
SET get_previous_date = NEW.date - interval 1 day;
UPDATE drt_summary_report SET drt_summary_report.lh_rendered_hours = drt_summary_report.lh_rendered_hours + rendered_hrs,
drt_summary_report.lh_rendered_hours_overlapp = drt_summary_report.lh_rendered_hours_overlapp + rendered_hrs_overlapped,
...
WHERE drt_summary_report.user_id=NEW.user_id AND drt_summary_report.login_date=NEW.date;
END IF;
UPDATE drt_summary_report SET drt_summary_report.dsh_rendered_hours = drt_summary_report.dsh_rendered_hours + rendered_hours,
drt_summary_report.dsh_rendered_hours_overlapp = drt_summary_report.dsh_rendered_hours_overlapp + rendered_hours_overlapped,
drt_summary_report.updated_at=now() 
WHERE drt_summary_report.user_id=NEW.user_id AND drt_summary_report.login_date=NEW.date;
ELSEIF(dtr_type = 'dlh')THEN 
          -- if the rest has registered on the same LH and SH then that value has to be set as null
...
WHERE drt_summary_report.user_id=NEW.user_id AND drt_summary_report.login_date=NEW.date;
END IF;
UPDATE drt_summary_report SET drt_summary_report.dlh_rendered_hours = drt_summary_report.dlh_rendered_hours + rendered_hrs,
drt_summary_report.dlh_rendered_hours_overlapp = drt_summary_report.dlh_rendered_hours_overlapp + rendered_hrs_overlapped,
drt_summary_report.updated_at=now()
WHERE drt_summary_report.user_id=NEW.user_id AND drt_summary_report.login_date=NEW.date;
ELSEIF(dtr_type = 'slh')THEN 
          -- if the rest has registered on the same LH and SH then that value has to be set as null
...
WHERE drt_summary_report.user_id=NEW.user_id AND drt_summary_report.login_date=NEW.date;
END IF;
UPDATE drt_summary_report SET drt_summary_report.slh_rendered_hours = drt_summary_report.slh_rendered_hours + rendered_hrs,
drt_summary_report.slh_rendered_hours_overlapp = drt_summary_report.slh_rendered_hours_overlapp + rendered_hrs_overlapped,
drt_summary_report.updated_at=now() 
WHERE drt_summary_report.user_id=NEW.user_id AND drt_summary_report.login_date=NEW.date;
END IF;
END IF;
