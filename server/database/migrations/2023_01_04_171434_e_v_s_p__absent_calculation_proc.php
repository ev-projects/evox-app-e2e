...
class EVSPAbsentCalculationProc extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        //
        DB::unprepared("
        CREATE  PROCEDURE EVSP_Absent_calculation(
            IN LoginDate DateTime,
            IN UserId BIGINT,
            IN IP_TimeLog varchar(20),
            OUT totalabsent decimal(10,2)
        )
        BEGIN
        ...
        UNIX_TIMESTAMP(CONVERT_TZ(FROM_UNIXTIME(end_flexy_datetime), @@session.time_zone, timelog))
         ...
         v_time_out,v_end_flexy_datetime FROM dtrs WHERE user_id=UserId and date=LoginDate;
        ...
        SET absent = 0;
          IF((v_timeid IS NULL AND v_time_out IS NULL) AND (v_start_datetime IS NOT NULL AND v_end_datetime IS NOT NULL) AND (leavecount <= 0) AND (holidaycount <= 0) AND dtr_type = 'reg' )THEN  
          SET absent = 1;
          END IF;
        SELECT absent INTO totalabsent;
        END");
    }
