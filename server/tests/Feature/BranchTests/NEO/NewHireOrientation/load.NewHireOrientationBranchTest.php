...
class NewHireOrientationLoadBranchTest extends TestCase
{
    ...
    public function index__load__has_record__returns_model()
    {
        ...
        if (!$survey) {
            $this->markTestIncomplete('no nho_survey rows in test DB');
        }
        $user = User::find($survey->user_id);
        if (!$user) {
            $this->markTestIncomplete('nho_survey row has no matching user');
        }
        $this->actingAs($user);
    ...
    public function index__load__no_record__returns_empty()
    {
        ...
        $user = User::where('is_active', 1)
            ...
            ->first();
        if (!$user) {
            $this->markTestIncomplete('no user without an nho_survey row in test DB');
        }
        $this->actingAs($user);
