 ...
 *   - catch(Exception): NOT observable as 400 — the catch's log_to_file() re-calls the (mocked, throwing)
 *     auth()->user(), re-throwing from inside catch -> uncaught 500 (see FINDING on the exception test).
 *
...
class COEFilterBranchTest extends TestCase
{
    ...
    // catch arm is NOT cleanly reachable when the failure source is auth itself.
    /** @test */
    public function getUsers__filter__exception__uncaught_500()
    {
        // FINDING: the intended catch->error_response(400) path cannot be observed here. getUsers() catches the
        // first auth()->user() Exception, but the catch body calls log_to_file('critical', ...), and log_to_file
        // (log_helper.php) itself calls auth()->user() again to build the "[UserID #: ...]" prefix. Since the mock
        // makes EVERY auth()->user() throw, that second call re-throws FROM INSIDE the catch block, escaping the
        // try/catch entirely -> uncaught 500. So an auth-sourced failure is not exception-safe: the error handler
        // re-invokes the very dependency that failed. 500 is the only reachable outcome for this trigger.
