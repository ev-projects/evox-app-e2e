...
class ChangeScheduleSubmitBranchTest extends TestCase
{
    ...
    /**
     * Valid ChangeScheduleRequest payload.
     * ChangeScheduleRequest::rules(StoreScheduleRequest $request) method-injects a FormRequest;
     * Laravel 5.7 resolves it via container->call(), which fires afterResolving -> validateResolved(),
     * so StoreScheduleRequest's whole rule set (name/source_type/schedule_type + inherited ScheduleRequest
     * schedule_details.all.* for a standard schedule) is CASCADE-validated against this same payload.
     * valid_from/valid_to alone => 422 before the mocked controller body runs, so all required fields
     * of the injected StoreScheduleRequest must be present too. (source_type=template keeps ScheduleRequest's
     * valid_* required_if inactive; the H:i times + <=1h break satisfy create_work_day_rule('all')/ValidBreakTime.)
     */
    ...
    public function store__submit__success__actor_owned_201()
    {
        ...
        $res->assertStatus(201)
            ...
            ->assertJson(['content' => ['user_id' => $this->user->id]]); // immune to payload impersonation (Laravel 5.7: assertJson, not assertJsonPath)
    }
