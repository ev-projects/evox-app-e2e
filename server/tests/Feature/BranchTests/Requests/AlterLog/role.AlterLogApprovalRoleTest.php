 ...
 * AlterLogController::approve/decline/pending/cancel authorization arms. Menu=Requests Page=AlterLog.
 *
 * WHAT THIS FILE PROVES (A27 authorization finding):
 *   AlterLogController::{approve,decline,pending,cancel} perform NO role-based authorization. No self-approval
 *   gate, no is_under_supervisee check, no admin gate. AlterLogRequest::authorize() returns true, and the
 *   alter_log routes carry NO permission middleware at all (app/Modules/Request/Routes/api.php lines
 *   125-134). Requester (glenn), correct supervisor (gary), unrelated supervisor (nidhi) and admin (dummyman)
 *   all reach business logic identically.
 *
 ...
 *     gate (status !== 403). approve()'s request_validity_checker() short-circuits SP-free on the far-past
 *     date 1900-01-01 (< -30 days).
 *
 * SKIPPED arms:
 *   // SKIPPED-SP approve() request_validity == 2 -> insertToAlterLogDispute() -> call_sp('EV_SP_PD_Autoamtion_AlterLog').
 *   // SKIPPED-SP success-200 render (all four actions): AlterLogResource serializes
 ...
 *   PUT /api/request/alter_log/approve|decline|pending|cancel/{id}
 */
...
class AlterLogApprovalRoleTest extends TestCase
{
    ...
    {
        return [
            'date'          => '1900-01-01',   // < -30 days -> request_validity_checker() SP-free, != 2
    ...
    private function approveAs(string $email)
    {
        ...
        return $this->putJson('/api/request/alter_log/approve/1', $this->payload());
    }
    ...
    private function declineAs(string $email)
    {
        ...
        return $this->putJson('/api/request/alter_log/decline/1', $this->payload());
    }
    ...
    {
        // FIXED A27: self-approval gate is now active in AlterLogController::approve() at controller level.
        // Glenn approving Glenn's own alter-log → 403 (Forbidden).
    ...
    // ============================================================ pending() ROLE MATRIX
    // SKIPPED-SP success-200 render: AlterLogResource fires call_sp('EH_SP_Direct_Supervisor') for
    ...
    public function pending__role__self__no_gate_reaches_biz_404()
    {
        ...
        $res = $this->putJson('/api/request/alter_log/pending/1');
    ...
    public function cancel__role__self__no_gate_reaches_biz_404()
    {
        ...
        $res = $this->putJson('/api/request/alter_log/cancel/1');
