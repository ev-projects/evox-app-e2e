 ...
 * FINDINGS: none in source. Test-helper note: deptSchedule() returns an anonymous Schedule subclass;
 *   Schedule::schedule_details()/schedule_policies() use hasMany(X::class) WITHOUT an explicit foreign
 *   key, so an anonymous subclass derives a garbage FK (class_basename of an anon class) -> QueryException
 *   during eager ScheduleResource serialization inside assign()'s try -> caught -> 400. The helper now
 *   pins 'schedule_id'/'id' on those two relations so the id-scoped reads stay empty & safe (200). The
 *   assignPayload was already complete/valid (validation never fails here — it would be 422, not 400).
 *
...
class ScheduleSubmitBranchTest extends TestCase
{
    ...
    private function deptSchedule($userDefaultSchedule): Schedule
    {
        ...
        $schedule = new class($dept) extends Schedule {
            ...
            // Schedule::schedule_details()/schedule_policies() call hasMany(X::class) with NO explicit
            // foreign key, so Eloquent derives it from class_basename($this). For an ANONYMOUS subclass
            // class_basename yields a garbage string (the file path), producing an invalid SQL column and
            // a QueryException. ScheduleResource serializes eagerly INSIDE success_response() (json_encode
            // in the JsonResponse ctor), which is inside assign()'s try -> the QueryException is caught ->
            // error_response 400. Pin the real 'schedule_id'/'id' keys so the read stays id-scoped & empty.
            public function schedule_details() {
                return $this->hasMany(\App\Modules\Schedule\Models\ScheduleDetail::class, 'schedule_id', 'id');
            }
            public function schedule_policies() {
                return $this->hasMany(\App\Modules\Schedule\Models\SchedulePolicy::class, 'schedule_id', 'id');
            }
        };
