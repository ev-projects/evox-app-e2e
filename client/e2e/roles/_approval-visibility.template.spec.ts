...
const ROLE = 'ph-supervisor';                    // matches the folder name / storageState file
const EXPECTS: Record<string, 'visible' | 'absent' | 'either'> = {
  myTeamRequests:   'visible',   // employees: 'absent' — the page itself should not be reachable
  approveControl:   'either',    // supervisors: 'either' (data-dependent) · employees: 'absent'
  bulkUpdate:       'visible',   // supervisors/admin: 'visible' · employees: 'absent'
  myOwnRequestPage: 'visible',   // everyone has their own requests page
  ownRequestApprove:'absent',    // NOBODY may approve their own request — the self-approval guard
};
// ─────────────────────────────────────────────────────────────────────────────
