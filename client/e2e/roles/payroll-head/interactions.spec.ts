...
test.use({ storageState: 'e2e/.auth/payroll-head.json' });

test.describe('payroll-head — Payroll Dispute Report (/app/payrolldisputeview/)', () => {
...
test.describe('payroll-head — open Dispute form WITHOUT saving (/app/payrolldispute/)', () => {
...
test.describe('payroll-head — Payroll Cutoff (/app/admin/PayrollCutoff/) view only', () => {
...
test.describe('payroll-head — DTR Summary (/app/team/DtrSummary) filters only', () => {
...
test.describe('payroll-head — India/Morocco Payroll Report (timeoff allocation)', () => {
  ...
  test('geo scoping: Morocco report route is reachable directly (menu only exposes India link)', async ({ page }) => {
    // FINDING: payroll-head's sidebar only shows one "India Payroll Report" link, but the
...
test.describe('payroll-head — Generate Date admin page (view only, direct nav)', () => {
