// Interaction spec — role: ph-employee (glenn.macasarte@eastvantage.com)
...
// or read directly from client source (DailyTimeRecord.js, MyRequests.js, QuickPunch.js). Selectors read
// from source only (not developer-vetted) are marked [SOURCE] and should be re-checked if a test fails.
...
test.describe('Dashboard (/app/Dashboard)', () => {
  ...
  test('QuickPunch widget shows Clock In / Clock Out — visibility only, NEVER clicked [SOURCE: QuickPunch.js]', async ({ page }) => {
    ...
    // Either both punch buttons render, or the rest-day note replaces Clock In (QuickPunch.js line 85) —
    // in both cases we only assert visibility, we do not click.
...
// =============================================================================
test.describe('Daily Time Record (/app/dtr/1593/)', () => {
  ...
  test('year react-select filter is visible [SOURCE: DailyTimeRecord.js name="year", class .year-dropdown]', async ({ page }) => {
    await page.goto('/app/dtr/1593/');
  ...
  test('DTR table renders with expected column headers [SOURCE: DailyTimeRecord.js render()]', async ({ page }) => {
    await page.goto('/app/dtr/1593/');
    const table = page.locator('table.dtr-table');
    const count = await table.count();
    // Table only renders once year+month+payroll cutoff are all selected (may auto-populate from
    // current_payroll_cutoff_ph on load) — guard for the case where filters aren't pre-selected.
    if (count > 0) {
  ...
  test('no Punch buttons are present on the DTR page itself [SOURCE: no QuickPunch import in DailyTimeRecord.js]', async ({ page }) => {
    await page.goto('/app/dtr/1593/');
...
test.describe('Request Form > Overtime (/app/request/Overtime/)', () => {
  ...
  test('filling the employee note does not submit the form ⚠️', async ({ page }) => {
    ...
    // still on the same request form — no navigation happened
...
test.describe('My Requests (/app/account/MyRequests)', () => {
  ...
  test('results table has an Actions column ⚠️ [SOURCE: MyRequests.js line 246]', async ({ page }) => {
    ...
    // (confirmed via a failed run's accessibility snapshot: the table WAS present with 4 rows and an
...
test.describe('DPA Webinar (/app/dpa)', () => {
  ...
  test('confirm checkbox and Submit button are never clicked, only asserted when visible ⚠️', async ({ page }) => {
    ...
    if (count > 0 && await checkbox.isVisible().catch(() => false)) {
      ...
      // NOT checked, NOT submitted — visibility assertion only
    }
  });
});
