// AUTO-GENERATED interaction spec — role: india-hr (toiba.qureshi@eastvantage.com)
...
test.describe('india-hr — Department Announcements (list + create form, no submit)', () => {
...
test.describe('india-hr — Employee List (filters only)', () => {
...
test.describe('india-hr — DPA List (filters only, no export)', () => {
...
test.describe('india-hr — Reports (Team Schedule / India Payroll Report)', () => {
  ...
  test('Morocco Payroll Report is NOT in the menu for an India HR', async ({ page }) => {
    await page.goto('/app/Dashboard', { waitUntil: 'load' });
    await page.getByRole('button', { name: 'Close' }).first().click({ timeout: 5000 }).catch(() => {});
    await expect(page.locator('aside.main-sidebar').getByText('Morocco Payroll Report', { exact: true })).toHaveCount(0);
  });
});

test.describe('india-hr — NEO Onboarding / Submission Report (view only, never Send Link)', () => {
...
test.describe('india-hr — COE HR (search + fill, no request submit)', () => {
...
test.describe('india-hr — Policies (view only, no upload)', () => {
...
test.describe('india-hr — OPS Functions (view only)', () => {
  test('Manage OPS Schedules list loads', async ({ page }) => {
    const resp = await page.goto('/app/ops/ManageOpsSchedulesList/', { waitUntil: 'load', timeout: 30000 });
    if (resp) expect(resp.status(), 'HTTP status').toBeLessThan(500);
  });
});

test.describe('india-hr — admin surface is NOT visible (role boundary)', () => {
