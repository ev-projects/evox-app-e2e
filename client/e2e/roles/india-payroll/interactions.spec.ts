// AUTO-GENERATED interaction spec — role: india-payroll (niveditha.sathyanarayana@eastvantage.com)
...
test.use({ storageState: 'e2e/.auth/india-payroll.json' });

test.describe('india-payroll — Payroll Dispute Report (/app/payrolldisputeview/)', () => {
...
test.describe('india-payroll — open Dispute form WITHOUT saving (/app/payrolldispute/)', () => {
...
test.describe('india-payroll — Payroll Cutoff (/app/admin/PayrollCutoff/) view only', () => {
...
test.describe('india-payroll — DTR Summary (/app/team/DtrSummary) filters only', () => {
...
test.describe('india-payroll — India Payroll Report (timeoff allocation, correct geo)', () => {
  ...
  test('geo scoping: Morocco report route is also reachable directly (no frontend geo lock found)', async ({ page }) => {
    // FINDING: india-payroll's sidebar correctly links to the India variant, but the Morocco
    // variant route is not blocked for this role at the frontend level either.
...
test.describe('india-payroll — Generate Date admin page (view only, direct nav)', () => {
