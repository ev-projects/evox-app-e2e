// AUTO-GENERATED from live menu dump — role: ph-payroll (susan.calderon@eastvantage.com)
...
const MENU: Array<{ label: string; href: string }> = [
  ...
  {
    "label": "Payroll Functions > Payroll Cutoff",
    "href": "/app/admin/PayrollCutoff/"
  }
];

test.describe('ph-payroll — menu traversal (' + MENU.length + ' pages)', () => {
  for (const item of MENU) {
    test(`loads: ${item.label} (${item.href})`, async ({ page }) => {
      ...
      for (const sig of ['Fatal error', 'Parse error', 'Uncaught Error', 'Whoops']) expect(body).not.toContain(sig);
