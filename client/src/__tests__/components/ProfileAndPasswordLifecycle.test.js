...
describe('PersonalInformation - saving basic information', () => {
    ...
    const renderPage = (props = {}) => {
        ...
        return { ...utils, updateUserProfile };
    };

    /* NOTE ON THE ID. PersonalInformation.js:49 dispatches against `profile.details.id`, while the
       ChangePassword panel dispatches against `props.user.id`. These tests cannot tell the two apart
       and do not claim to: the Save button and the Change Password button only render when
       `user.id === profile.details.id` (line 203), so in every configuration a user can reach, the
       two ids are the same value. The assertions pin the id that is dispatched, not its source. */
    test('a confirmed save sends every field plus the PUT method override', async () => {
        const { container, updateUserProfile } = renderPage();
        type(container, 'first_name', 'Juanito');
        type(container, 'mobile_number', '09998887777');
        await submit(container);

        expect(confirmSpy).toHaveBeenCalledWith('Are you sure you want to save these changes?');
        expect(updateUserProfile).toHaveBeenCalledTimes(1);
        expect(updateUserProfile).toHaveBeenCalledWith(5, {
            first_name: 'Juanito',
            last_name: 'Dela Cruz',
            email: 'juan@evox.test',
    ...
    test('declining the confirmation abandons the save', async () => {
        confirmSpy.mockReturnValue(false);
        const { container, updateUserProfile } = renderPage();
        type(container, 'first_name', 'Juanito');
        await submit(container);
    ...
    test('a blank first name blocks the save before the confirmation dialog', async () => {
        const { container, getByText, updateUserProfile } = renderPage();
        type(container, 'first_name', '');
        await submit(container);
    ...
    test('a first name shorter than three characters is rejected', async () => {
        const { container, getByText, updateUserProfile } = renderPage();
        type(container, 'first_name', 'Jo');
        await submit(container);

        expect(getByText('3 Minimum Characters')).toBeInTheDocument();
        expect(updateUserProfile).not.toHaveBeenCalled();
    });

    test('a malformed email address is rejected', async () => {
        const { container, getByText, updateUserProfile } = renderPage();
        type(container, 'email', 'not-an-email');
        await submit(container);

        expect(getByText('Not a valid email address.')).toBeInTheDocument();
        expect(updateUserProfile).not.toHaveBeenCalled();
    });

    test('a blank mobile number is rejected', async () => {
        const { container, getByText, updateUserProfile } = renderPage();
        type(container, 'mobile_number', '');
        await submit(container);
    ...
    test('a profile that arrives with no stored names starts blank and reports all four fields as required', async () => {
        const { container, getAllByText, updateUserProfile } = renderPage({
            profile: { details: { id: 5 }, closeAllForm: false },
        });
        expect(field(container, 'first_name').value).toBe('');
        await submit(container);
    ...
    test('an unchanged but complete profile still saves the stored values under the PUT override', async () => {
        const { container, updateUserProfile } = renderPage();
        await submit(container);
