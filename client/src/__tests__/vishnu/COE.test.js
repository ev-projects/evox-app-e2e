...
const defaultProps = {
    ...
    // state.coe
    instance: [],          // state.coe.instance — past COE records
    ...
    purpose_index: null,
    // Component reads the purposes list from this.props.constant.COE_PURPOSES (nested),
    // not the top-level prop above.
    constant: {
        COE_PURPOSES: [
            { purpose: 'Proof of Employment' },
            { purpose: 'Loan Application' },
    ...
    // Action-creator props (connect is mocked to passthrough, so these must be supplied).
    // COE/COEHR call this.props.fetchCOE() on mount and this.props.addCOE() on submit.
    fetchCOE: jest.fn(),
    addCOE:   jest.fn(),
    // Router / navigation
    dispatch: jest.fn(),
...
describe('COEHR (HR Certificate of Employment) component', () => {
    ...
    // SKIP: staging COEHR renders `user={this.props.instance.user}` with no null-guard,
    // so an undefined `instance` genuinely throws. The "does not crash" premise does not
    // hold against the actual code (it has no resilience to undefined instance).
    test.skip('does not crash when instance is undefined', () => {
