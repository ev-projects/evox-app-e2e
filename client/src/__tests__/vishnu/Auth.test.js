...
// ModalLogin imports { Spring } from 'react-spring/renderprops'; react-spring 8's
// renderprops subpath doesn't interop under Jest (Spring resolves undefined), so stub it
// as a render-prop component that renders its children.
jest.mock('react-spring/renderprops', () => ({
    __esModule: true,
    Spring: ({ children }) => (typeof children === 'function' ? children({}) : (children || null)),
}));
...
describe('Login component', () => {
    ...
    test('renders Log In button', () => {
        ...
        // The page has three "Log In" buttons (email, Google, Eastvantage email).
        expect(getAllByText(/Log In/i).length).toBeGreaterThanOrEqual(1);
    });
