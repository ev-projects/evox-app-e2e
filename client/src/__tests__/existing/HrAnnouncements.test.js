...
const legacyListDefaultProps = {
    ...baseProps,
    // Component reads this.props.hrAnnouncement as the list array (mapStateToProps maps
    // it to state.hrAnnouncement.listInstance) and only renders the table (with the
    // "Title" header) when hrAnnouncement?.length > 0.
    hrAnnouncement: [
        { id: 1, title: 'Legacy Announcement', log_date: '2026-07-01', user: { first_name: 'HR', last_name: 'Admin' } },
    ],
    listInstance:         [],
...
function renderList(props = {}) {
    const merged = { ...listDefaultProps, ...props };
    // The component reads the redux slice this.props.departmentAnnouncement.*,
    // so mirror the flat list/loaded overrides into that nested object.
    merged.departmentAnnouncement = {
        isDepartmentAnnouncementListLoaded: merged.isDepartmentAnnouncementListLoaded,
        depAnnouncementlist: merged.depAnnouncementlist,
    };
    return render(
        <MemoryRouter>
            <HrAnnouncementsList {...merged} />
        </MemoryRouter>
    );
}
...
describe('HrAnnouncementsList component', () => {
    ...
    // Component maps depAnnouncementlist without a null guard (HrAnnouncementsList.js:61),
    // so it genuinely throws on null — staging code lacks this defensive behavior.
    test.skip('does not crash with null announcement list', () => {
