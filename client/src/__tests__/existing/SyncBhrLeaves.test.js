...
/**
 * Admin/SyncBhrLeaves — admin tool to pull BambooHR leave records for a date range.
 * Formik form + a results table over `sync.biometrics`. connect stripped;
 * Wrapper/AdminLte/BackButton/DatePicker stubbed.
 * Source: src/container/Admin/SyncBhrLeaves/SyncBhrLeaves.js
 */
...
import SyncBhrLeaves from '../../container/Admin/SyncBhrLeaves/SyncBhrLeaves';
...
describe('SyncBhrLeaves container', () => {
    ...
    test('renders the Sync BHR Leaves title', () => {
        const { getByText } = renderComponent();
        expect(getByText(/Sync BHR Leaves/i)).toBeInTheDocument();
    });
    ...
    test('does not crash when the sync result set is populated', () => {
        expect(() => renderComponent({
            sync: { leaves: [{ id: 1, name: 'Emp One', leave_date: '2026-07-01' }] },
        })).not.toThrow();
    });
});
