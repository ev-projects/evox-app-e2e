...
// RegisterUser imports MultiSelect as a DEFAULT export; provide both default and named.
jest.mock('react-multi-select-component', () => {
    const stub = ({ labelledBy }) => <div aria-label={labelledBy}>MultiSelect</div>;
...
jest.mock('formik', () => {
    ...
    return {
        ...
        Field: ({ name, type, placeholder, children }) => (
            // Some components use <Field>{({field,form}) => (...)}</Field> render-props;
            // invoke the children function so their inner inputs (with placeholders) render.
...
const registerUserDefaultProps = {
    ...
    isSuccessful:    false,
    roles:           [],
    department:      [],   // component reads this.props.department (singular)
    departments:     [],
    registerUser:        jest.fn(),
    fetchDepartmentList: jest.fn(),   // called in componentDidMount/willMount
...
const assignFeatureDefaultProps = {
    ...
    location:           { search: '' },
    isLoading:          false,
    isUserListLoaded:            true,   // gate: search/feature UI renders only when loaded
...
const assignDeptHandlersDefaultProps = {
    ...
    supervisor:                 [],
    client:                     [],
    department:                 [],   // guard: renders form only when department && supervisor defined
    departments:                [],
...
describe('AssignDepartmentHandlers component', () => {
    ...
    test('renders Departments label', () => {
        const { getByText } = renderAssignDeptHandlers();
        expect(getByText(/Departments/i)).toBeInTheDocument();
    });
...
const assignEmpClientDefaultProps = {
    ...
    employee:                   [],
    client:                     [],
    department:                 [],   // guard: renders form only when department && employee defined
    departments:                [],
    department_users:           [],
...
describe.skip('AssignEmployeesClient component — CLIENT MODULE REMOVED 2026-08-10', () => {
    ...
    test('renders Client List label', () => {
        const { getByText } = renderAssignEmpClient();
