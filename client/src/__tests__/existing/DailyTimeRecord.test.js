...
jest.mock('../../components/GridComponent/AdminLte.js', () => ({
    ...
    // Content renders the page title/subtitle via props (not children) in the real component.
    Content:          ({ children, title, subtitle }) => <div><h1>{title}</h1><div>{subtitle}</div>{children}</div>,
...
// DtrPunch renders RecentPunch and MultiQuickpunch — connected components that read
// this.props.dashboard.recent_punch from the store. Under the passthrough connect mock they
// receive no props and crash, so stub them (they are not exercised directly by these tests).
jest.mock('../../components/Dashboard/PunchComponents/RecentPunch', () => () => <div data-testid="recent-punch" />);
jest.mock('../../components/Dashboard/PunchComponents/MultiQuickpunch', () => () => <div data-testid="multi-quickpunch" />);
...
jest.mock(
    '../../components/Dashboard/PunchComponents/MultiQuickpunch/MultiQuickpunch',
    () => () => <div data-testid="multi-quickpunch">MultiQuickpunch</div>,
    { virtual: true }
);
jest.mock(
    '../../components/Dashboard/PunchComponents/RecentPunch/RecentPunch',
