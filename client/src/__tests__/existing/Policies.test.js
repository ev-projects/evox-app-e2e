...
jest.mock('../../container/PageLoadingCard.js/PageLoadingCard.js', () => () => <div>Loading...</div>);
...
describe('PoliciesDocumentUpload component', () => {
    ...
    test('renders Upload submit button', () => {
        const { getAllByText } = renderUpload();
        // "Upload" appears in the heading and the submit control — assert at least one.
        expect(getAllByText(/Upload/i).length).toBeGreaterThanOrEqual(1);
    });
