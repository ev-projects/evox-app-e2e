...
jest.mock('../../components/GridComponent/AdminLte.js', () => ({
    ContainerHeader:  ({ children, title }) => <div>{title}{children}</div>,
    // Content receives the page heading via `title` and the Add button via `subtitle`
...
jest.mock('react-data-table-component', () => ({
    ...
    default: ({ columns, data, progressPending }) => (
        <div data-testid="data-table">
            ...
            <table>
                <thead>
                    <tr>{Array.isArray(columns) && columns.map((col, i) => <th key={i}>{col.name}</th>)}</tr>
                </thead>
                <tbody>
                    {/* B-003: listInstance initial value is {} (not an array), so guard with Array.isArray */}
                    {Array.isArray(data) && data.map((row, i) => <tr key={i}><td>{row.name}</td></tr>)}
                </tbody>
            </table>
        </div>
    ),
...
jest.mock('react-bootstrap', () => ({
    Form:     Object.assign(
        ({ children }) => <form>{children}</form>,
        {
            ...
            // Control also exposes Form.Control.Feedback (used by PayrollCutoffForm)
    ...
    Col:      ({ children }) => <div>{children}</div>,
    Row:      ({ children }) => <div>{children}</div>,
    // Components import { Form, Button, InputGroup, FormControl } from 'react-bootstrap'
    InputGroup:  ({ children }) => <div>{children}</div>,
    FormControl: ({ name, placeholder, value, onChange }) => (
        <input name={name} placeholder={placeholder} value={value || ''} onChange={onChange || (() => {})} />
    ),
    Modal:    Object.assign(
