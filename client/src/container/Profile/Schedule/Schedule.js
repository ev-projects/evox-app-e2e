...
const Schedule = (props) => {
  ...
  const handleChangeDate = (start_date, end_date, scope_type) => {
    ...
    if (scope_type == "month") {
      props.setDateList(
        getDaysArrayInMonth(start_date.format("YYYY"), start_date.format("M"))
      );
      props.setWeekList(
        generateWeekList(start_date.format("YYYY"), start_date.format("M"))
      );
    }
    ...
    if (scope_type == "custom") {
      ...
      props.setWeekList({
        week_list: custom.week_list,
        dates_list: custom.dates_list,
      });
    }
  };

  return Validator.isValid(profile) ? (
    <Wrapper>
          ...
          {profile.scope == "week" ? (
            <React.Fragment>
              <WeekTeamSchedule
                dtr={dtr}
                data={profile.dates}
                date_list={profile.date_list}
                schedule={profile.schedule}
                temporary_schedule={profile.temporary_schedule}
              />
            </React.Fragment>
      ...
      {week.length > 0 ? (
        <React.Fragment>
          {week.map((value, index) => {
            ...
            var flex_time_in =
              schedule?.schedule_details?.all?.start_flexy_time;
            var flex_time_out = schedule?.schedule_details?.all?.end_flexy_time;
            if (
              schedule?.rest_day?.includes(
                moment(value).format("ddd").toLowerCase()
              )
            ) {
              details = "rest_day";
            } else {
            ...
            if (week.length == dtr.length) {
              ...
              flex_time_in = moment(dtr[index].start_flexy_datetime).format(
                "HH:mm"
              );
              flex_time_out = moment(dtr[index].end_flexy_datetime).format(
                "HH:mm"
              );
              return (
                <Col>
                  <div className="schedule-date">
                    {moment(date_list[index]).format("MMM DD")}
                  </div>

                  <Card>
                    <div className={"card-body " + details}>
                      <div class="schedule_info">
                        {dtr[index].attendance_status.name == "Rest Day" ? (
                          "REST DAY"
                        ) : (
                          <div>
              ...
              if (dtr[index]) {
                ...
                temporary_schedule.map((temp) => {
                  if (
                    temp.valid_from == temp.valid_to &&
                    moment(date_list[index]).isSame(temp.valid_from)
                  ) {
                    time_in = temp?.schedule_details?.all?.start_time;
                    time_out = temp?.schedule_details?.all?.end_time;
                    flex_time_in =
                      temp?.schedule_details?.all?.start_flexy_time;
                  ...
                  if (temp.valid_from != temp.valid_to) {
                    if (
                      moment(date_list[index]).isBetween(
                        temp.valid_from,
                        temp.valid_to
                      )
                    ) {
                      ...
                      flex_time_in =
                        temp?.schedule_details?.all?.start_flexy_time;
                      flex_time_out =
                        temp?.schedule_details?.all?.end_flexy_time;
                      rest_day = temp.rest_day;
                    } else {
                      if (
                        moment(date_list[index]).isSame(temp.valid_from) ||
                        moment(date_list[index]).isSame(temp.valid_to)
                      ) {
                        time_in = temp?.schedule_details?.all?.start_time;
                        time_out = temp?.schedule_details?.all?.end_time;
                        flex_time_in =
        ...
        <React.Fragment>
          {data.map((week, week_index) => {
            ...
            if (length == week_index) {
              ...
              for (
                var i = 6;
                i > week_dictionary[[week_list[length][1]]];
                i--
              ) {
                last_cols = (
                  <React.Fragment>
            ...
            return (
              <React.Fragment>
                <Row className="emp_sched profile">
                  {" "}
                  {first_week_offset}{" "}
                  {week.map((day, day_index) => {
                    ...
                    var flex_time_in =
                      schedule?.schedule_details?.all?.start_flexy_time;
                    var flex_time_out =
                      schedule?.schedule_details?.all?.end_flexy_time;
                    if (
                      schedule?.rest_day?.includes(
                    ...
                    if (date_list.length == dtr.length) {
                      if (
                        dtr[day_number - 1].attendance_status.name == "Rest Day"
                      ) {
                        details = "rest_day";
                      } else {
                        details = "early";
                      ...
                      time_in = moment(
                        dtr[day_number - 1].start_datetime
                      ).format("HH:mm");
                      time_out = moment(
                        dtr[day_number - 1].end_datetime
                      ).format("HH:mm");
                      flex_time_in = moment(
                        dtr[day_number - 1].start_flexy_datetime
                      ).format("HH:mm");
                      flex_time_out = moment(
                        dtr[day_number - 1].end_flexy_datetime
                      ).format("HH:mm");
                      return (
                        <Col>
                          {moment(date_list[day_number - 1]).format("MMM, D")}

                          <Card>
                            <div className={"card-body " + details}>
                              <div class="schedule_info">
                                {dtr[day_number - 1].attendance_status.name ==
                                "Rest Day" ? (
                                  "REST DAY"
                                ) : (
                                  <div>
                      ...
                      if (dtr[day_number - 1]) {
                        ...
                        temporary_schedule.map((temp) => {
                          if (
                            temp.valid_from == temp.valid_to &&
                            moment(date_list[day_number - 1]).isSame(
                              temp.valid_from
                            )
                          ) {
                            time_in = temp?.schedule_details?.all?.start_time;
                          ...
                          if (temp.valid_from != temp.valid_to) {
                            if (
                              moment(date_list[day_number - 1]).isBetween(
                                temp.valid_from,
                                temp.valid_to
                              )
                            ) {
                              ...
                              if (
                                ...
                                time_in =
                                  temp?.schedule_details?.all?.start_time;
                                time_out =
                                  temp?.schedule_details?.all?.end_time;
                                flex_time_in =
                                  temp?.schedule_details?.all?.start_flexy_time;
                                flex_time_out =
                                  temp?.schedule_details?.all?.end_flexy_time;
...
const mapDispatchToProps = (dispatch) => {
  return {
    fetchTimeOff: (id, start_date, end_date) =>
      dispatch(fetchTimeOff(id, start_date, end_date)),
    ...
    viewEmployeeDtr: (user_id, from, to) =>
      dispatch(viewEmployeeDtr(user_id, from, to)),
  };
};
