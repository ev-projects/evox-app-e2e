...
class RecentDtrNavCopy extends Component {
	constructor(props){
    	super(props);
      this.timer = 0;
    	this.state = {
...
canClockOut(clock_in) {
  ...
  {recent_dtr.length > 0 ?<>{recent_dtr.slice().reverse().map((dtr, index) => {
                  ...
                  {dtr.dtr_history.length > 0 ? <>
                    {dtr.dtr_history.slice().reverse().map((history, index) => {
                  // let dtr_type = dtr.attendance_status.slug;
                  // let status =  <div>
                  //                 <div className={dtr.attendance_status.slug}>{dtr.attendance_status.name}</div>
                  //                 <div>{DtrFormatter.displayHoliday(dtr.holidays)}</div>
                  ...
                  return<>
                    <div>{history.time_in}--{history.time_out}</div>
                  </>
                })}
  ...
  export default connect(mapStateToProps, mapDispatchToProps)(RecentDtrNavCopy);
