#!/usr/bin/env bash
# run-e2e-with-coverage.sh
# EVOX-29 | Phase 3: Test Environment + Anonymized DB
#
# Orchestrates Playwright E2E tests alongside PHPUnit backend coverage.
# Run from the evox-app root or from the scripts/ directory.
# See scripts/SETUP.md for one-time installation requirements.

set -euo pipefail

# Load NVM so npx is available in non-interactive SSH sessions
export NVM_DIR="${HOME}/.nvm"
# shellcheck disable=SC1091
[ -s "$NVM_DIR/nvm.sh" ] && . "$NVM_DIR/nvm.sh"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
SERVER_DIR="$REPO_ROOT/server"
CLIENT_DIR="$REPO_ROOT/client"
COVERAGE_DIR="$REPO_ROOT/coverage"

log() { echo "[$(date '+%Y-%m-%d %H:%M:%S')] $*"; }
log_sep() { log "────────────────────────────────────────────────────"; }

log "======================================================"
log "  EVOX: E2E + Backend Coverage (EVOX-29)"
log "======================================================"
log "Repo root : $REPO_ROOT"
log "Server    : $SERVER_DIR"
log "Client    : $CLIENT_DIR"
log "Coverage  : $COVERAGE_DIR"
log_sep

# ── Clean previous artifacts ──────────────────────────────────────────────────
log "Cleaning previous coverage artifacts..."
rm -rf \
    "$COVERAGE_DIR/html" \
    "$COVERAGE_DIR/clover.xml" \
    "$COVERAGE_DIR/phpunit-coverage.php" \
    "$COVERAGE_DIR/phpunit.log" \
    "$COVERAGE_DIR/playwright-report" \
    "$COVERAGE_DIR/playwright-results.xml" \
    "$COVERAGE_DIR/playwright.log"

mkdir -p "$COVERAGE_DIR"

# ── PHPUnit with PCOV coverage ────────────────────────────────────────────────
log "Step 1/2: Running PHPUnit with PCOV coverage..."
cd "$SERVER_DIR"

PHPUNIT_CMD="./vendor/bin/phpunit"

if ! command -v "$PHPUNIT_CMD" &>/dev/null && [ ! -f "$PHPUNIT_CMD" ]; then
    log "ERROR: PHPUnit not found. Run 'composer install' in server/."
    exit 1
fi

# PCOV_ENABLED is informational; PCOV activates when the extension is loaded.
# If PCOV/Xdebug are absent, coverage flags are ignored and tests still run.
if php -m | grep -qi pcov; then
    log "PCOV extension detected — collecting coverage."
    $PHPUNIT_CMD \
        --coverage-php  "$COVERAGE_DIR/phpunit-coverage.php" \
        --coverage-html "$COVERAGE_DIR/html" \
        --coverage-clover "$COVERAGE_DIR/clover.xml" \
        2>&1 | tee "$COVERAGE_DIR/phpunit.log"
else
    log "WARN: PCOV not loaded — running PHPUnit without coverage."
    log "      See scripts/SETUP.md to install PCOV."
    $PHPUNIT_CMD 2>&1 | tee "$COVERAGE_DIR/phpunit.log"
fi

log "PHPUnit: complete."
log_sep

# ── Playwright E2E ────────────────────────────────────────────────────────────
log "Step 2/2: Running Playwright E2E tests..."
cd "$CLIENT_DIR"

if ! command -v npx &>/dev/null; then
    log "ERROR: npx not found. Node.js ≥14 is required."
    exit 1
fi

if [ ! -d "node_modules/@playwright" ]; then
    log "WARN: Playwright not installed. Run 'npm install && npx playwright install chromium' in client/."
    log "      See scripts/SETUP.md for details."
    exit 1
fi

# Use node v18 directly — system node (v12) is too old for Playwright
NODE18="$NVM_DIR/versions/node/v18.20.8/bin/node"
[ ! -f "$NODE18" ] && NODE18="$(which node)"

"$NODE18" node_modules/.bin/playwright test \
    --config=playwright.config.ts \
    2>&1 | tee "$COVERAGE_DIR/playwright.log"

log "Playwright: complete."
log_sep

# ── Merge coverage ────────────────────────────────────────────────────────────
if [ -f "$COVERAGE_DIR/phpunit-coverage.php" ]; then
    log "Merging coverage reports..."
    php "$SCRIPT_DIR/merge-coverage.php"
else
    log "Coverage merge skipped (no PHPUnit coverage data — PCOV not available)."
fi

log_sep
log "Done."
log "  Backend HTML : $COVERAGE_DIR/html/index.html"
log "  Backend XML  : $COVERAGE_DIR/clover.xml"
log "  E2E HTML     : $COVERAGE_DIR/playwright-report/index.html"
log "  E2E XML      : $COVERAGE_DIR/playwright-results.xml"
